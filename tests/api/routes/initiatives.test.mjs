...
/** Queue the three reads the handler makes after the auth lookup. */
function queueReads({ initiatives = [initiative()], projects = [PROJECT], meta = META } = {}) {
...
describe('initiatives read', () => {
  ...
  it('queries the initiative partition and the project partition', async () => {
    ...
    const queries = mockSend.mock.calls
      .map(([cmd]) => cmd)
      .filter((cmd) => cmd?.type === 'Query');
    const wanted = queries.map((q) => q.params.ExpressionAttributeValues[':t']);

    expect(wanted).toContain(RECORD_INITIATIVE);
    expect(wanted).toContain(RECORD_PROJECT);
  });
